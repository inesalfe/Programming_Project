...
void draw_plot(	cairo_t *cr, RingBuffer * x_rb, RingBuffer * y_rb, 
	...
	for(i=0, idx = (x_rb->head + start - 1) % x_rb->capacity; i < len - 1; i++, idx = (idx + 1) % x_rb->capacity){
...
int main(int argc, char** argv){
	...
	label_status = gtk_label_new("Time Scale for Plot 1 and 2");
	...
	label_plot_scale = gtk_label_new("Time Scale for Plot 1 and 2");
	...
	adjustment_M 			= (GtkAdjustment *) gtk_adjustment_new (0.300, 0.15, 1.50, 0.050, 1, 0.0);
	adjustment_m 			= (GtkAdjustment *) gtk_adjustment_new (0.200, 0.15, 1.50, 0.050, 1, 0.0);
	...
	adjustment_theta 		= (GtkAdjustment *) gtk_adjustment_new (0.785, -1.570, 1.570, 0.001, 1, 0.0);
	adjustment_x 			= (GtkAdjustment *) gtk_adjustment_new (0.100, -0.150, 0.150, 0.05, 1, 0.0);
	adjustment_omega 		= (GtkAdjustment *) gtk_adjustment_new (0.000, -2.000, 2.000, 0.100, 1, 0.0);
	adjustment_v 			= (GtkAdjustment *) gtk_adjustment_new (0.000, -2.000, 2.000, 0.100, 1, 0.0);
